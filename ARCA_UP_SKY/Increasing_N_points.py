...
def check(energy,theta):
        ...
        for ang in range(len(theta)-1):
                if theta[ang]>90:
                        binwidth=(np.radians(theta[ang+1])-np.radians(theta[ang]))
...
Nevent2=plt.figure()
ax40=Nevent2.add_subplot(131)
ax40.plot(zenithy,sum_of_events(TeVSky,zenithy),'+',markersize=2,color='r')
for ene in TeVSky:
        ax40.plot(zenithy,Numb_events(ene,zenithy),'+',markersize=2)
ax40.set_xscale('log')
ax41=Nevent2.add_subplot(132)
ax41.plot(zenithy,sum_of_events(PeVSky,zenithy),'+',markersize=2,color='r')
for ene in PeVSky:
        ax41.plot(zenithy,Numb_events(ene,zenithy),'+',markersize=2)
ax41.set_xscale('log')

ax42=Nevent2.add_subplot(133)
ax42.plot(zenithy,sum_of_events(EeVSky,zenithy),'+',markersize=2,color='r')
for ene in EeVSky:
        ax42.plot(zenithy,Numb_events(ene,zenithy),'+',markersize=2)
ax42.set_xscale('log')
